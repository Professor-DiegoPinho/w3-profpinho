...
Nos artigos anteriores, você aprendeu sobre estruturas condicionais usando `if`, `elif` e `else`. Agora, vamos explorar uma forma diferente de tomar decisões no código com a estrutura `match`.
...
A estrutura `match` funciona de maneira semelhante a um "interruptor" (_switch_ em inglês) encontrado em outras linguagens de programação.
...
- Quando encontra o primeiro case compatível, ele executa o bloco e para a busca.
...
Em muitas situações, você vai querer realizar uma ação padrão se nenhum dos valores definidos para os `case` corresponder ao `match`. Para isso, podemos usar o `_`, que funciona como um coringa, pois corresponde a qualquer valor que esteja no `match`.
...
Nem sempre o valor da expressão é o suficiente para tomar a decisão. Quando você precisar de mais condições, você também pode adicioná-las com um `if` dentro do próprio `case`. Essas condições são chamadas de _guards_.

Imagine que um carro autônomo, aqueles que não precisam de um motorista para circular. Quando ele está se movendo pelas ruas, ele verifica se o sinal de trânsito está verde.
...
Aprendemos que o Python oferece duas formas principais de lidar com decisões no código: `if/elif/else` e `match`. Embora os dois resolvam problemas parecidos, cada um brilha em situações diferentes.
...
No fim, a escolha não é sobre qual é "melhor", mas sim qual é mais adequada para cada caso. Entender quando cada uma se encaixa é o que melhora a qualidade do seu código.
