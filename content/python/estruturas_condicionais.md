...
Você já viu como o Python representa valores (`int`, `float`, `str`, `bool`) e como usamos diversos operadores para trabalhar com eles. Agora vamos dar um passo importante: **fazer o código tomar decisões**.
...
Em muitas linguagens, blocos de código são delimitados com chaves (`{}`). Em Python, quem manda é a **indentação**, lembra dela? Como vimos no artigo "Sintaxe Básica", elas são os espaços ou tabulações no começo de uma linha de código.

Logo abaixo do `if`, tudo estiver mais à direita está indentado e faz parte do bloco condicional. Se você tirar a indentação, o Python entende que a linha **não faz parte** do `if`:
...
> Dica: no terminal, o próprio Python já coloca a indentação para você quando você começa um bloco `if`. Mas quando você usar outros programas feitos para escrever código, fique de olho. Depois de escolher se vai usar **espaços ou tab**, mantenha sempre o mesmo padrão dentro do bloco. Nada de misturar os dois!
...
Se a condição do `if` não for verdadeira, o código vai obrigatoriamente entrar no `else`. Ele equivale à palavra SENÃO que usamos em português para falar sobre uma condição, como quando dizemos: "Se fizer sol, eu vou à praia. Senão, vou no shopping".
...
if nota >= 90:                        # testa a 1ª condição
    ...
    print('Nota B')
else:                                 # se todas as anteriores forem falsas, entra neste bloco
...
- O `else` deve aparecer por último, depois de todos os `elif`.
...
- O `elif` é opcional. Só precisamos dele se quisermos testar condições que não seriam atendidas pelo `if` nem pelo `else`.
...
Não se preocupe se parecer que são muitas informações. Vamos continuar vendo estruturas condicionais no próximo artigo. Quanto mais você estudar esse assunto, mais fácil ele vai ficar.
